...
namespace Supermarket_management.Controllers
{
    public class Product : Controller
    {
        ...
        {
            var spQuery = _context.SanPhams.Include(s => s.MaDanhMucNavigation).AsQueryable();

            // Nếu có tham số maDanhMuc → lọc theo danh mục
            if (maDanhMuc.HasValue)
            {
                spQuery = spQuery.Where(sp => sp.MaDanhMuc == maDanhMuc.Value);
            }

            // Gửi danh sách danh mục để dùng trong dropdown (nếu muốn hiển thị)
            ViewBag.DanhMucs = _context.DanhMucs.ToList();

            return View(spQuery.ToList());
        }
